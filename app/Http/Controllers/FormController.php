...
class FormController extends Controller
{
    ...
    //create_form_calculation
    public function create_form_calculation()
    {
        return view('admin.pages.form.cal_create');
    }
    ...
    public function list_of_form()
    {
        ...
        // $data['forms'] = DB::table('forms')->orderBy('id', 'desc')->where('status', true)->join('forms', 'forms.created_by', '=', 'users.id')->where('form_creator', Auth::user()->id)->get();


        if (Auth::user()->hasRole('super-admin')) {
            $data['forms'] = DB::table('forms')->orderBy('id', 'desc')->join('saved_format', 'saved_format.saved_form_id', '=', 'forms.id')->get();
        } elseif (Auth::user()->user_type == 'AUTHORIZE_USER') {
            $data['forms'] = DB::table('forms')->orderBy('id', 'desc')->join('saved_format', 'saved_format.saved_form_id', '=', 'forms.id')->where('form_creator', Auth::user()->id)->get();
        } else {
            $data['forms'] = [];
        }

        // elseif (Auth::user()->user_type == 'AUTHORIZE_USER') {
        //     $data['forms'] = DB::table('forms')->orderBy('id', 'desc')->where('status', true)->where('form_creator', Auth::user()->id)->get();
        // }
        // echo '<pre>';
        // print_r($data['forms']);
        // die;
    ...
    public function reportClassify($form_id)
    {
    ...
    public function new_list_of_report()
    {
        if (Auth::user()->hasRole('super-admin')) {
            $data['forms'] = DB::table('forms')->orderBy('id', 'desc')->join('saved_format', 'saved_format.saved_form_id', '=', 'forms.id')->get();
        } elseif (Auth::user()->user_type == 'AUTHORIZE_USER') {

            $data['forms'] = DB::table('forms_info_all')->join('forms', 'forms.id', '=', 'forms_info_all.form_id')->join('saved_format', 'saved_format.saved_form_id', '=', 'forms_info_all.form_id')->where('form_created_by', Auth::user()->id)->orwhere('sender_id', Auth::user()->id)->orwhere('receiver_id', Auth::user()->id)->get()->unique('form_name');
        } else {
            $data['forms'] = [];
        }

        return view('admin.pages.form.new_list_of_report', $data);
    }
    ...
    public function list_of_report($id)
    {

        SwitchLanguage(Session::get('language'));
        $user = new UserForm();
        $getUser = $user->getUserId(Auth::user()->user_category < 7);
        $user_category = range(Auth::user()->user_category, 6);
        //   for ($i=Auth::user()->user_category; $i < 6; $i++) { 
        //   return gettype($i);
        //   }
        //   die;
        $user_cat = array_values($user_category);
        $user_range = DB::table('users')->whereIn('user_category', $user_category)->select('id')->get();
        $array_user = json_decode(json_encode($user_range, true), true);

        //  $get_val = array_values($array_user);
        ...
        // // ->where('sub_form_id', $data['form_tables'][0]->form_id)
        //         $users = DB::table('users')->whereIn('user_category',array(Auth::user()->user_category, Auth::user()->user_category + 1))->join('forms','forms.created_by','=','users.id')->get();

        // if (Auth::user()->user_category == 1) {
        //     // $data['data_value'] = DB::table($data['table_info'])->where('created_by',Auth::user()->id)->distinct()->get(['created_by']);
        //     // $data['data_value'] = DB::table('users')->join('countries', 'countries.country_id', '=', 'leagues.country_id')->distinct()->get(['created_by']);
        //     $data['data_value'] = DB::table($data['table_info'])->distinct()->get(['created_by']);
        // }else{
        //     // $data['data_value'] = DB::table($data['table_info'])->distinct()->get(['created_by']);
        //     $data['data_value'] = DB::table($data['table_info'])->whereIn('created_by', $array_user)->distinct()->get(['created_by']);
        // }

        // $data['data_value'] = DB::table($data['table_info'])->whereIn('created_by', $array_user)->join('form_tables', 'form_tables.table_name','=','')->distinct()->get(['created_by']);
        // $data['data_value'] = DB::table($data['table_info'])->whereIn('created_by',$array_user)->join('user_forms', 'user_forms.reciever_user_id','=', "" . $data['table_info'] . "".'.'.'created_by')->distinct()->get(['created_by']);
        // $data['data_value'] = DB::table($data['table_info'])->whereIn('created_by', $array_user)->join('user_forms', 'user_forms.reciever_user_id', '=', "" . $data['table_info'] . "" . '.' . 'created_by')->distinct()->get(['created_by']);
        $data['data_value'] = DB::table($data['table_info'])->whereIn('authorize_user', $array_user)->distinct()->get(['authorize_user']);

        $data['data_value_duration'] = DB::table($data['table_info'])->whereIn('created_by', $array_user)->join('user_forms', 'user_forms.reciever_user_id', '=', "" . $data['table_info'] . "" . '.' . 'created_by')->distinct()->get(['duration']);


        // $data['newsss'] = DB::table('user_forms')->join('forms','forms.id','=','user_forms.sub_form_id')->whereIn('reciever_user_id', $array_user)->orwhereIn('sender_user_id', $array_user)->distinct()->get(['sub_form_id']);

        // $data['data_count'] = DB::table($data['table_info'])->distinct()->get(['created_by'])->count();
        $data['data_count'] = DB::table($data['table_info'])->distinct()->get(['created_by'])->count();
        ...
        return view('admin.pages.form.list_of_report', $data);
    }
    ...
    public function deleteForm($id)
    {
        ...
        $data = DB::table('forms')->where('id', $id)->update(['status' => false]);
    }
    ...
    public function formTracker($id)
    {
        $data['all_div'] = DB::table('alternative_role_levels')->get();
        $data['forms_info'] = DB::table('forms_info_all')->where('form_id',$id)->where('report_status', 'RECIEVED')->distinct()->get(['sender_category']);
        $data['sender_array'] = json_decode(json_encode($data['forms_info']), true);
        $data['column_array'] = array_column($data['sender_array'], 'sender_category');
        $data['users_get'] = DB::table('users')->whereIn('user_category', $data['column_array'])->where('user_type', 'AUTHORIZE_USER')->get();
        $data['table'] = DB::table('form_tables')->where('form_id',$id)->get();
        $data['sender_array_form'] = json_decode(json_encode($data['users_get']), true);
        $data['column_array_form'] = array_column($data['sender_array_form'], 'id');
        $data['users_get_form'] = DB::table('forms_info_all')->where('form_id',$id)->whereIn('sender_id', $data['column_array_form'])->distinct()->get(['sender_id']);
        $data['table'] = DB::table('form_tables')->where('form_id', $id)->get();
        $table = $data['table'][0]->table_name;
        $data['info'] = DB::table($table)->get();

        //$data['dash'] = DB::table('users')->whereIn()
        // echo '<pre>';
        // print_r($data['users_get_form']); die;
       return view('admin.pages.form.form_tracker',$data);
    }
    ...
    //         DB::table('form_tables')->insert(['form_id' => $form_id, 'table_name' => $final_row]);
    //         $columns = '';
    ...
    //           $columns .=   $second . '  VARCHAR(250)  NOT NULL, ';
    //         }

    //         // $tblName = str_replace(' ', '_', $rowTable);
    ...
    // 	//             $createTableSqlString = "CREATE TABLE $tblName (
    //   //   id INT(11) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    ...
    {
        //   print_r($request->all());die;
        $form_id = DB::table('forms')->insertGetId(['form_name' => $request->form_name, 'form_id' => $request->form_id, 'html_form' => $request->html_form, 'form_creator' => Auth::user()->id]);

        //all info

        DB::table('forms_info_all')->insert([
            'form_created_by'  => Auth::user()->id,
            'creator_category' => Auth::user()->user_category,
            'form_id'          => $form_id,
        ...
        // DB::table('forms_info_all')->insert(['form_id', $request->form_id,'form_created_by',Auth::user()->id,'creator_category',Auth::user()->user_category,'report_status','created']);
        ...
        foreach ($request->table_name as $rowTable) {
            ...
            $final_row = str_replace(' ', '_', $first_row);
            DB::table('form_tables')->insert(['form_id' => $form_id, 'table_name' => $final_row, 'table_headline' => $request->table_headline, 'created_by' => Auth::user()->id]);
            $columns = '';
            $columnData = 'table_' . $sl . '_column_name';
            foreach ($request->$columnData as $rowColumn) {
                $first = str_replace(' ', '_', $rowColumn);
    ...
    //create table form calculation

    public function create_form_table_calculation(Request $request)
    {
        //   print_r($request->all());die;
        $form_id = DB::table('forms')->insertGetId(['form_name' => $request->form_name, 'form_id' => $request->form_id, 'html_form' => $request->html_form, 'form_creator' => Auth::user()->id]);
    ...
    public function tableCreate(Request $request)
    {
        ...
        $start = $request->id;
        $tough = $request->all();
        $array = array_slice($tough, 1);
        $remove_array = array(

            'id' => $start
        );
        $new_array = array_diff_assoc($array, $remove_array);
        ...
        // $named = DB::table('user_forms')->where('sub_form_id', $request->var_id)->where('reciever_user_id', Auth::user()->id)->orWhere('sender_user_id', $request->form_creator);
        // $find_creator = DB::table('forms')->where('id', $request->var_id)->get();
        // $creator = $find_creator[0]->form_creator;
        // $to_user_data = $creator;
        // $sendNotification = Auth::user()->name . " has updated the form.";
        // $link = URL::to('/') . '/form/' . $request->var_id . '';
    ...
    public function created_submit_form($id, $def = null)
    {
        ...
        $users_category = DB::table('users')->select('id')->whereIn('user_category', array(Auth::user()->user_category, Auth::user()->user_category + 1))->get();
        $users_array = json_decode(json_encode($users_category, true), true);


        $form_creator = $this->formCreator($id);
        ...
        $data['form'] = DB::table('forms')->where('id', $id)->get();
        // $data['userforms'][0]->sub_form_id;
        $data['form_tables'] = DB::table('form_tables')->where('form_id', $id)->get();
        $data['generated_form'] = DB::table('forms')->where('id', $id)->get();
        $data['table_info'] = $data['form_tables'][0]->table_name;
        $data['form_creator_check'] = $data['form_tables'][0]->created_by;
        $data['data_value'] = DB::table($data['table_info'])->whereIn('created_by', $users_array)->get();
        $data['saved_format_type'] = DB::table('saved_format')->where('saved_table_name', $data['table_info'])->get();


        if (Auth::user()->hasRole('super-admin')) {
            $data['data_value'] = DB::table($data['table_info'])->where('authorize_user', '!=', 0)->get();
        } elseif ($data['form_creator_check']  == Auth::user()->id) {
            $data['data_value'] = DB::table($data['table_info'])->where('filled_by', '!=', 0)->get();
        } elseif (Auth::user()->user_type == 'ENTRY_USER') {
            $data['data_value'] = DB::table($data['table_info'])->where('filled_by', Auth::user()->id)->get();
        } elseif (Auth::user()->user_type == 'AUTHORIZE_USER') {
            $data['data_value'] = DB::table($data['table_info'])->where('authorize_user', Auth::user()->id)->get();
        }



        $data['get_column'] = DB::table('saved_format')->select('saved_column')->where('saved_table_name', $data['table_info'])->first();
        $data['fixed_column'] = $data['get_column']->saved_column;


        // echo '<pre>';
        // print_r($data['fixed_column']->saved_column);
        // die;


        // $data['meta_value'] = DB::table($data['table_info'])->select('name')->where('created_by', 248)->where('filled_by','=',0)->get();
        // $sinus = json_decode(json_encode($data['meta_value']), true);
        // $data['dove'] = array_column($sinus,'name');
        //    echo Auth::user()->id;
        //    print_r($data['data_value']);die;



        // $data['data_value'] = DB::table($data['table_info'])->whereget();
        // $data['test'] = DB::table($data['table_info'])->where_not_null()->get;
        //$data['date_submission'] = DB::table('user_forms')->where('reciever_user_id', Auth::user()->id)->where('sub_form_id', '=', $id)->get();


        /*  */
        // $data['formsupdate'] = DB::table('forms_info_all')->where('form_id', $id)->where('form_created_by', $form_creator)->where('report_status', 'RECIEVED')->where('sender_id', Auth::user()->id)->orwhere('receiver_category', Auth::user()->user_category)->orwhere('receiver_id',Auth::user()->id)->where('sender_category','=', 'receiver_category')->get();

        // $sender_id   = $data['userforms'][0]->sender_user_id;
        // $sender_category   = $tasks_controller->getUserCategory($sender_id);
        // $reciever_id = $data['userforms'][0]->reciever_user_id;
        // $reciever_category = $tasks_controller->getUserCategory($reciever_id);



        //$data['formsupdate'] = DB::table('forms_info_all')->where('sender_category', Auth::user()->user_category)->where('receiver_category', Auth::user()->user_category)->where('form_id', $id)->where('form_created_by', $form_creator)->where('sender_id',$sender_id)->where('receiver_id', $reciever_id)->where('report_status', 'RECIEVED')->get();


        //8/4/22
        // $data['formsupdate'] = DB::table('forms_info_all')->where('form_id', $id)->where('form_created_by', $form_creator)->whereIn('sender_category', array(Auth::user()->user_category, Auth::user()->user_category - 1 , Auth::user()->user_category + 1))->where('receiver_category', Auth::user()->user_category)->where('report_status', 'RECIEVED')->get();

        // $data['lob'] = DB::table('table_9352285')->where('filled_by',242)->where('authorize_user',241)->sum('naam');
        // $data['lob'] = DB::table('table_9352285')->where('filled_by',242)->where('authorize_user',241)->sum(DB::raw('naam + role + section'));
        ...
        //                 // $data['formsupdate'] = DB::table('auth_entry_table')->where('ae_form_id', $id)->where('ae_sender_id', Auth::user()->user_creator)->where('ae_receiver_id', Auth::user()->id)->get();
        //                 // $data['date_submission'] = DB::table('auth_entry_table')->where('ae_form_id', $id)->where('ae_sender_id', Auth::user()->user_creator)->where('ae_receiver_id', Auth::user()->id)->get();

        //                 // $data['formsupdate'] = DB::table('forms_info_all')->where('form_id', $id)->where('form_created_by', $form_creator)->whereIn('sender_category', array(Auth::user()->user_category, Auth::user()->user_category - 1 , Auth::user()->user_category + 1))->where('receiver_category', Auth::user()->user_category)->where('report_status', 'RECIEVED')->get();

        //             } elseif (Auth::user()->user_type == 'AUTHORIZE_USER' AND $form_creator = Auth::user()->id) {
        //                 $getEntryUser = $this->getEntryUser(Auth::user()->id);
        //                 $data['formsupdate'] = DB::table('auth_entry_table')->where('ae_form_id', $id)->where('ae_sender_id', Auth::user()->id)->where('ae_receiver_id', $getEntryUser)->get();
        //                 $data['date_submission'] = DB::table('auth_entry_table')->where('ae_form_id', $id)->where('ae_sender_id', Auth::user()->id)->where('ae_receiver_id', $getEntryUser)->get();
        //             }
        //         } else {
        //             $data['formsupdate'] = DB::table('forms_info_all')->get();
        //             $data['date_submission'] = DB::table('forms_info_all')->get();
        //         }
        ...
        // $ff = DB::table("table_9835162")->select('name')->where('authorize_user','=', 0)->get();
        // $jj =   json_decode(json_encode($ff), true);
        // $gg = (object) $jj;
        // $kp = json_decode(json_encode($gg), true);
        // $ll = array_column($kp, 'name');
        // $guti = array_values($ll);
        // $obss = (object)$guti;
        ...
        // $jjj =   json_decode(json_encode($ffss), true);
        // $ggg = (object) $jjj;
        // $kpp = json_decode(json_encode($ggg), true);



        //$jfff = array_replace($lll, $lll ,$ll);



        // $ff = DB::table("table_9835162")->update(['name' => 1],['name' => 2],['name' => 3]);



        // echo '<pre>';

        // //return gettype($kp);die;
        // print_r($guti);
        // die;
        // $adata = (object) $ll;
        // $calm=  str_replace($ffss, $ff, $ffss);
        ...
        $tasks_controller = new UserForm();
        $get_form_id = $id;
        $form_info = DB::table('forms')->where('id', $id)->get();
        ...
        //     'receiver_id' => $tasks_controller->getAuthorizeuser(Auth::user()->id),
        //     'receiver_category' => $tasks_controller->getUserCategory(Auth::user()->id),
        ...
        if (Auth::user()->user_type == 'AUTHORIZE_USER') {
            $entry = $this->getEntryUser(Auth::user()->id);
            $data['final_status'] = DB::table('forms_info_all')->where('form_id', $id)->where('sender_id', Auth::user()->id)->where('receiver_id', $entry)->where('report_status', 'RECIEVED')->get();
            if (Auth::user()->user_category == 1) {
                $rec_cateogry = Auth::user()->user_category;
            }else{
                $rec_cateogry = Auth::user()->user_category - 1;
            }
            
            $rec_id = DB::table('users')->where('user_category', $rec_cateogry)->get();
            $to_user_data = $rec_id[0]->id;
            $sendNotification = Auth::user()->name . "Your Form Has Been Approved";
            $link = URL::to('/') . '/created-submit-form/' . $id;
            event(new Notification(\Illuminate\Support\Facades\Auth::user()->id, $to_user_data, $sendNotification, $link));
    
        }elseif(Auth::user()->user_type == 'ENTRY_USER'){
            
            $auth  = Auth::user()->user_creator;
            $data['final_status'] = DB::table('forms_info_all')->where('form_id', $id)->where('sender_id', $auth)->where('receiver_id', Auth::user()->id)->where('report_status', 'RECIEVED')->get();
            ...
            // echo '<pre>';
            // print_r($data['final_status']);die;

        if (Auth::user()->user_category != 1) {


            $data['formsupdate'] = DB::table('forms_info_all')->where('form_id', $id)->where('form_created_by', $form_creator)->whereIn('sender_category', array(Auth::user()->user_category, Auth::user()->user_category - 1, Auth::user()->user_category + 1))->orwhere('receiver_category', Auth::user()->user_category)->where('report_status', 'RECIEVED')->get();

            $data['date_submission'] = DB::table('forms_info_all')->get();

            // echo '<pre>';
            // print_r($data['formsupdate']);die;
        } else {
            $data['formsupdate'] = DB::table('forms_info_all')->get();
            $data['date_submission'] = DB::table('forms_info_all')->get();
            ...
            $data['date_submission'] = DB::table('forms_info_all')->where('form_id', $id)->where('form_created_by', $form_creator)->where('send_process', 'individual')->get();
        }


        if (Auth::user()->user_category == 1) {
            $rec_id = DB::table('users')->where('user_category', 1)->get();
            $to_user_data = $rec_id[0]->id;
            $sendNotification = Auth::user()->name . " has backed the form";
            $link = URL::to('/') . '/created-submit-form/' . $id;
            event(new Notification(\Illuminate\Support\Facades\Auth::user()->id, $to_user_data, $sendNotification, $link));
        } else {
            $rec_cateogry = Auth::user()->user_category - 1;
    ...
    public function formCreator($id)
    {

        $form = DB::table('forms_info_all')->where('form_id', $id)->get();
        return $form[0]->form_created_by;
    }

    public function getEntryUser($creator_id)
    {

        $entry_id = DB::table('users')->where('user_creator', $creator_id)->where('user_type', 'ENTRY_USER')->get();
        return $entry_id[0]->id;
    }
    // public function getAuthUser($creator_id)
    // {

    //     $auth_id = DB::table('users')->where('id', Auth::user()->id)->where('user_type', 'AUTHORIZE_USER')->get();
    //     return $auth_id[0]->user_creator;
    // }


    /* approvalCheck */

    public function approvalCheck($user, $formid = null)
    {
        // $data['formsupdate']  = DB::table('user_forms')->join('user_forms','forms.id','=', 'user_forms.sub_form_id')->get();
        //$data['formsupdate']  = DB::table('user_forms')->where('sub_form_id',$formid)->where('reciever_user_id',Auth::user()->id)->get();
        // $current_user =  $data['formsupdate'][0]->reciever_user_id;die;

        // $data['formsupdate'] = DB::table('forms')->where('id', $formid)->get();
        // $creator_id = $data['formsupdate'][0]->form_creator;
        // $data['user_check'] = DB::table('users')->where('id', $creator_id)->where()->get();
        // print_r($data['user_check'][0]->user_category);
        // die;
        //$data['formsupdate'] = DB::table('forms')->where('id',$formid)->where('form_creator',$user)->update(['status' => true]);
        // $data['formsupdate'] = DB::table('forms')->where('id',$formid)->update(['status' => true]);


        $form_creator = $this->formCreator($formid);
        $getEntryUser = $this->getEntryUser(Auth::user()->id);
        // $data['formscheck'] = DB::table('forms')->where('id',$formid)->where('form_creator',$user)->get();
        $data['userforms'] = DB::table('user_forms')->where('sub_form_id', $formid)->get();
        // $data['formsupdate'] = DB::table('forms_info_all')->where('form_id', $formid)->where('form_created_by', $form_creator)->where('report_status', 'RECIEVED')->where('sender_id', Auth::user()->id)->where('receiver_category', Auth::user()->user_category)->update(['final_status' => true, 'feedback_msg' => null]);

        // $data['formsupdate'] = DB::table('auth_entry_table')->where('ae_form_id', $formid)->where('ae_sender_id', Auth::user()->id)->where('ae_receiver_id', $getEntryUser)->update(['ae_approval' => true]);

        $data['formsupdate'] = DB::table('forms_info_all')->where('form_id', $formid)->where('form_created_by', $form_creator)->where('report_status', 'RECIEVED')->where('sender_id', Auth::user()->id)->where('receiver_category', Auth::user()->user_category)->update(['final_status' => true, 'feedback_msg' => null
        ]);
        $sendNotification = "Your Report Has Been approved";
        $link = URL::to('') . '/created-submit-form/' . $formid;
        event(new Notification(\Illuminate\Support\Facades\Auth::user()->id, $data['userforms'][0]->reciever_user_id, $sendNotification, $link));
    }

    public function declineFeedback($user, $formid = null, $msg = null)
    {
        $form_creator = $this->formCreator($formid);
        $getEntryUser = $this->getEntryUser(Auth::user()->id);
        //$data['formsupdate'] = DB::table('forms')->where('id', $formid)->where('form_creator', $user)->update(['message' => $msg]);
        // $data['formsupdate'] = DB::table('forms')->where('id', $formid)->where('form_creator', $user)->update(['status' => false]);
        // $data['formsupdate'] = DB::table('forms')->where('id', $formid)->update(['message' => $msg]);
        // $data['formsupdate'] = DB::table('forms')->where('id', $formid)->update(['status' => false]);
        // $data['formsupdate'] = DB::table('forms_info_all')->where('form_id', $formid)->where('form_created_by', $form_creator)->where('sender_id', Auth::user()->id)->where('report_status', 'RECIEVED')->where('sender_id', Auth::user()->id)->where('receiver_category', Auth::user()->user_category)->update(['final_status' => false, 'feedback_msg' => $msg]);

        // $data['formsupdate'] = DB::table('auth_entry_table')->where('ae_form_id', $formid)->where('ae_sender_id', Auth::user()->id)->where('ae_receiver_id', $getEntryUser)->update(['ae_approval' => false, 'ae_comment' => $msg]);

        $data['formsupdate'] = DB::table('forms_info_all')->where('form_id', $formid)->where('form_created_by', $form_creator)->where('sender_id', Auth::user()->id)->where('report_status', 'RECIEVED')->where('sender_id', Auth::user()->id)->where('receiver_category', Auth::user()->user_category)->update(['final_status' => false, 'feedback_msg' => $msg]);

        $data['userforms'] = DB::table('user_forms')->where('sub_form_id', $formid)->get();
        $sendNotification = "Your Report Has Been Rejected";
        $link = URL::to('') . '/created-submit-form/' . $formid;
        event(new Notification(\Illuminate\Support\Facades\Auth::user()->id, $data['userforms'][0]->reciever_user_id, $sendNotification, $link));
    }
    ...
    public function edit_created_form($id)
    {
        $data['userforms'] = DB::table('user_forms')->where('reciever_user_id', Auth::user()->id)->orWhere('sender_user_id', Auth::user()->id)->where('sub_form_id', '=', $id)->get();
        $data['form_tables'] = DB::table('form_tables')->where('form_id', $id)->get();
        $data['generated_form'] = DB::table('forms')->where('id', $id)->get();
        $data['table_info'] = $data['form_tables'][0]->table_name;
        $data['data_value'] = DB::table($data['table_info'])->get();
        return view('admin.pages.form.submit_form.edit_submit_form', $data);
    }
    ...
    public function view_form($id, $def = null)
    {
        ...
        SwitchLanguage(Session::get('language'));
        $users_category = DB::table('users')->select('id')->whereIn('user_category', array(Auth::user()->user_category, Auth::user()->user_category + 1, 1))->get();
        $users_category_single = DB::table('users')->select('id')->whereIn('user_category', array(Auth::user()->user_category))->where('user_type', 'ENTRY_USER')->get();
        $users_array_single = json_decode(json_encode($users_category_single, true), true);;
        $users_array = json_decode(json_encode($users_category, true), true);
        $data['form'] = DB::table('forms')->where('id', $id)->get();
        $data['form_tables'] = DB::table('form_tables')->where('form_id', $id)->get();
        $data['table_info'] = $data['form_tables'][0]->table_name;
        if ($data['form'][0]->form_creator == Auth::user()->id) {
            $data['data_value'] = DB::table($data['table_info'])->where('authorize_user', 0)->where('filled_by', 0)->get();
            $data['data_value_count'] = DB::table($data['table_info'])->where('authorize_user', 0)->where('filled_by', 0)->count();
            $data['comment'] = DB::table('forms_info_all')->where('form_id', $id)->where('report_status', 'RECIEVED')->where('send_process', 'back')->get();
            $data['saved_table'] = DB::table('forms_info_all')->where('form_id', $id)->where('report_status', 'CREATED')->get();
        } else {
            $data['data_value'] = DB::table($data['table_info'])->whereIn('created_by', $users_array)->get();
            $data['data_value_count'] = DB::table($data['table_info'])->where('authorize_user', 0)->where('filled_by', 0)->count();
        }

        $cols = DB::getSchemaBuilder()->getColumnListing($data['table_info']);
        $sliced_col = array_slice($cols, 6);
        $data['final_fixed'] = $sliced_col[0];


        $data['default_form_check_count'] = DB::table('new_default_form')->where('default_form_id', $id)->count();
        $data['default_form_check'] = DB::table('new_default_form')->where('default_form_id', $id)->get();



        $data['status_check'] =  DB::table('forms_info_all')->where('form_id', $id)->where('report_status', 'RECIEVED')->where('receiver_id', Auth::user()->id)->whereNotNull('deadline')->get();

        $data['data_rem'] = json_decode(json_encode($data['data_value'], true), true);
        //print_r($data['data_value']);die;
        return view('admin.pages.form.view', $data);
    }
    /* view form calculation*/

    public function view_form_calc($id, $def = null)
    {
        ...
        // $data['fixed_column'] = DB::table($data['table_info'])->count();
        // $first_object = json_decode(json_encode($data['fixed_column'][0]), FALSE);
        // $returnArrr = (array)$first_object;
        // $fixed_key  = array_keys($returnArrr);
        // $data['sliced']     = array_slice($fixed_key,6);
        // $data['final_fixed']= $data['sliced'][0];
        // return $ssss;die;
        ...
        $data['data_rem'] = json_decode(json_encode($data['data_value'], true), true);
        //print_r($data['data_value']);die;
        return view('admin.pages.form.view_calc', $data);
    }


    public function created_latest_form($id, $def = null)
    {
        SwitchLanguage(Session::get('language'));
        ...
        $data['table_info'] = $data['form_tables'][0]->table_name;
        // echo "<pre>";
        // print_r($data['table_info']);
        // die;
        $data['data_value'] = DB::table($data['table_info'])->get();
        $data['data_next'] = DB::getSchemaBuilder()->getColumnListing($data['table_info']);
        //$data['data_array']  =   json_decode(json_encode($data['data_next'][0]), true);
        // $data['first']       = array_keys($data['data_array']);
        $data['next']        = array_slice($data['data_next'], 3, -1);

        //return $data['next'];

        $data['data_rem'] = json_decode(json_encode($data['data_value'], true), true);

        return view('admin.pages.form.formstructure', $data);
    }
    ...
    function deleteColumn($del_id, $table_name)
    {
        $data = DB::table($table_name)->where('id', $del_id)->delete();
    }


    public function individualReport($id, $tab)
    {
        // return $id;
        SwitchLanguage(Session::get('language'));
        $data['form'] = DB::table('forms')->where('id', $tab)->get();
        $data['form_tables'] = DB::table('form_tables')->where('form_id', $tab)->get();
        $data['table_info'] = $data['form_tables'][0]->table_name;


        $data['data_value'] = DB::table($data['table_info'])->where('authorize_user', $id)->latest()->get();
        $data['data_info']   = DB::table('forms_info_all')->where('form_id', $tab)->where('final_status', true)->get();
        // echo '<pre>';
        // print_r($data['data_info']);
        // die;
        $data['data_rem'] = json_decode(json_encode($data['data_value'], true), true);
        return view('admin.pages.form.individual', $data);
    }
    public function compiledReport($form_id)
    {
        // return $id;
        SwitchLanguage(Session::get('language'));
        $data['form'] = DB::table('forms')->where('id', $form_id)->get();
        $data['form_tables'] = DB::table('form_tables')->where('form_id', $form_id)->get();
        $data['table_info'] = $data['form_tables'][0]->table_name;


        $data['data_value'] = DB::table($data['table_info'])->where('authorize_user', '!=', 0)->where('filled_by', '!=', 0)->latest()->get();
        $data['data_info']   = DB::table('forms_info_all')->where('form_id', $form_id)->where('final_status', true)->get();
        // echo '<pre>';
        // print_r($data['data_info']);
        // die;
        $data['data_rem'] = json_decode(json_encode($data['data_value'], true), true);
        return view('admin.pages.form.compiled', $data);
    }
    ...
    public function generate_pdf($id)
    {
        ...
        $mpdf->writeHTML($this->pdfHTML($id));
        return $mpdf->Output('createdtable.pdf', 'D');
    ...
    public function pdfHTML($id)
    {
        ...
        $data['userforms'] = DB::table('user_forms')->where('sub_form_id', '=', $id)->get();

        return view('myPDF', $data);
    }

    public function generate_pdf_individual($id, $tab)
    {
        $data = [
            'foo' => 'bar'
        ];
        ...
        $dmkkamdf = $mpdf->writeHTML($this->pdfhtmlndividual($id, $tab));
        return $mpdf->Output('createdtable.pdf', 'D');
    }


    public function pdfhtmlndividual($id, $tab)
    {
        SwitchLanguage(Session::get('language'));
        $data['form'] = DB::table('forms')->where('id', $tab)->get();
        $data['form_tables'] = DB::table('form_tables')->where('form_id', $tab)->get();
        ...
        $data['data_value'] = DB::table($data['table_info'])->where('authorize_user', $id)->latest()->get();
        $data['data_rem'] = json_decode(json_encode($data['data_value'], true), true);
        return view('individualPdf', $data);
    }
    ...
    public function generated_update(Request $request)
    {
        ...
        // $laaz = array_slice($dam,4);

        $djj = explode(",", $request->colmns);
        $meow = array_slice($djj, 1);

        $total_row = $request->row_id;
        // print_r();
        ...
        // print_r($dam);
        // die;
        foreach (array_unique($request->rowsk) as $key => $value) {
            foreach ($meow as $gheow) {
                $ho = $gheow . '_' . $value;
                $id_get = 'id_' . $value;
                $table_get = 'table_name_' . $value;
                $get_id = $request->$id_get;
                $data[$gheow] = $request->$ho;
                DB::table($request->$table_get)->where('id', $get_id)->update($data);


                // DB::statement('UPDATE  SET id = $get_id');
        ...
        DB::table('auth_entry_table')->where('ae_form_id', $request->form_id_get)->where('ae_receiver_id', Auth::user()->id)->update(['ae_status' => true]);

        // $named = DB::table('forms_info_all')->where('form_id', $request->form_id_get)->where('receiver_id', Auth::user()->id)->orWhere('sender_id', Auth::user()->id)->get();
        // $to_user_data = User::find($named[0]->sender_user_id);
        // $sendNotification = Auth::user()->name . " has updated the form.";
    ...
    function table_name_row_check($table_name)
    {

        $row_count = DB::table($table_name)->count();
        return $row_count;
    }
    ...
    function savedFormat(Request $request)
    {
        $tab_name = $request->saved_table_name;



        /* fixed column */
        $table_info = DB::table($tab_name)->where('created_by', Auth::user()->id)->where('filled_by', 0)->get();
        $da = json_decode(json_encode($table_info), true);
        $keys = array_keys($da[0]);
        $splice = array_splice($keys, 6);
        $last_element = array_pop($splice);
        $string =   implode(",", $keys);
        $object = (object) $splice;
        $form_id = $this->getFormId($tab_name);

        /* update forms info all */
        ...
        /* existance and format checl */

        $row_count = DB::table('saved_format')->where('saved_table_name', $tab_name)->count();
        $exisistance_check = DB::table('saved_format')->where('saved_table_name', $tab_name)->exists();

        if ($exisistance_check == 0 && $row_count == 0) {
            DB::table('saved_format')->insert([
                'saved_table_name' => $request->saved_table_name,
                'saved_form_creator' => $request->saved_form_creator,
                'saved_form_type' => $request->saved_form_type,
                'saved_format' => $request->saved_format,
                'saved_column' => $request->saved_column,
                'saved_form_id' => $request->saved_form_id,
            ]);
            DB::table('forms_info_all')->where('form_id', $form_id)->where('report_status', 'CREATED')->update(['final_status' => true]);
        } elseif ($exisistance_check == 1 && $row_count == 1) {

            DB::table('saved_format')->where('saved_table_name', $tab_name)->update([
                'saved_format' => $request->saved_format,
            ]);
            DB::table('forms_info_all')->where('form_id', $form_id)->where('report_status', 'CREATED')->update(['final_status' => true]);
        }
    ...
    function SaveDefaultForm(Request $request)
    {
        $row_count = DB::table('new_default_form')->where('default_form_id', $request->default_form_id)->count();
        $exisistance_check = DB::table('new_default_form')->where('default_form_id', $request->default_form_id)->exists();
        if ($exisistance_check == 0 && $row_count == 0) {
            DB::table('new_default_form')->insert([
                'default_form_id' => $request->default_form_id,
                'default_table_name' => $request->default_table_name,
                'default_created_by' => $request->default_created_by,
                'default_form_status' => 1
    ...
    {
        $data['user_id'] = DB::table('userforms')->where('reciever_user_id', Auth::user()->id)->get();

        // print_r($data['user_id']);die;
        return response()->json($data['user_id'][0]->id);
    }
    ...
    {
        $get_col_data   =  $request->all();
        $slice_col_data   = array_slice($get_col_data, 1, -1);
        $slice_col_key    = array_keys($slice_col_data);
        $slice_col_value  = array_values($slice_col_data);

        $table_name = $request->table_name;


        $old_col = $slice_col_key[0];
        $new_col = $slice_col_value[0];


        $table_new = str_replace(' ', '_', $table_name);
        $old_col_new = str_replace(' ', '_', $old_col);
        $new_col_new = str_replace(' ', '_', $new_col);
        $new_col_str = str_replace(array(
            '\'', '"', '\r\n',
            ',', ';', '<', '>', '(', ')', '-', '=', '+'
        ), ' ', $new_col_new);


        $sql = "ALTER TABLE `$table_new` CHANGE COLUMN $old_col_new $new_col_str LONGTEXT";
    ...
    public function columnCreate(Request $request)
    {
        $get_col_data      = $request->all();
        $table_name        = $request->table_name;
        $slice_col_data    = array_slice($get_col_data, 1, -2);
        $slice_after_table = array_slice($get_col_data, 2, -1);
        $new_column        = array_values($slice_col_data);
        $after_column      = array_values($slice_after_table);
        $column_string            = $new_column[0];
        $column          = str_replace(' ', '_', $column_string);
        $after             = $after_column[0];;
        $sql = "ALTER TABLE $table_name ADD $column LONGTEXT NULL AFTER $after";
        DB::statement($sql);
        //   return redirect()->back();
    ...
    public function columnDelete($colname, $table_name)
    {

        // $new_table_name = str_replace(array('\'', '"', ',', ';', '<', '>', '(', ')', '-', '=', '+' ), ' ', $table_name);

        // $new_col_name = str_replace(array('\'', '"', ',', ';', '<', '>', '(', ')', '-', '=', '+' ), ' ', $colname);
    ...
    public function afterColumn($tablename)
    {
        $tab = str_replace(' ', '', $tablename);
        $data['prologue']   = DB::table($tab)->get();
        $data['monologue']  =   json_decode(json_encode($data['prologue'][0]), true);
        $data['next']       = array_keys($data['monologue']);
    ...
    public function reminder_notification($userId, $formId)
    {

        $get_entry_user = new UserForm();
        ...
        $entry_user = $get_entry_user->getEntryUser($userId);

        $to_user_data = User::find($entry_user);
        $sendNotification = Auth::user()->name . "Please Submitform";
        // $link = URL::to('/').'/created-submit-form/' . $sub_form;
        $link = URL::to('/') . '/form/' . $formId;
    ...
    public function selectFormFormat()
    {
        return view('admin.pages.form.select_type_form');
    }
