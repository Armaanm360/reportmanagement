...
<div class="content">
    ...
    <div class="page-inner">
        ...
        <form action="{{ url('update-form-1.3') }}" id="submitForm" method="POST">
            @csrf

             <input type="hidden" name="author_id" value="{{ Auth::user()->id }}">
            <div class="rel-26">
                <h3>
                    <input type="hidden" name="years" id="yearTextInput" value=""><span id="yearText">
                   
                    
                    @foreach ($years as $years)
                    {{ $years -> years }}@endforeach</span> অর্থবছরের বাজেট বাস্তবায়ন অগ্রগতি</h3>
        ...
        //                  $fakir = $split_url[0].'/'.$split_url[1].'/';


        //   echo   $direction = $fakir.'indiv-stored-forms/';
                ...
                <table border="1" width="100%" id="myTable">
                      <thead>
                        ...
                        <tr>
                            <td><span id="yearText">@foreach ($budyears as $news) {{ $news -> years }}@endforeach</span></td>
                            <td>প্রকৃত ব্যয়</td>
                     ...
                     <tbody>
                        ...
                        <tr class="row1">
                            ...
                            <td class="column1 style7 s" @if($sub_rows != 0) rowspan="{{$sub_rows}}" @endif><textarea type="text" name="budget_{{$row->id}}" id="" cols="30" rows="10">{{$row->budget}}</textarea></td>
                        ...
                        @if ($subRow ->branch_responsible_implementation == Auth::user()->division )
                          ...
                          <input type="hidden" name="sub_row_id[]" value="{{$subRow->id}}">
                           <input type="hidden" name="unique_form_id" value="{{$subRow->unique_form_id}}">
                            ...
                            <td class="column7 style6 s"><input type="text" readonly  id="" value="{{$divisioninfo->divisionName($row -> branch_responsible_implementation)[0]['name']}}">
                            <input type="hidden" name="sub_row_branch_responsible_implementation_{{$subRow->id}}" value="{{$divisioninfo->divisionName($row -> branch_responsible_implementation)[0]['id']}}">
                            </td>
                          </tr>
                           @endif
                          @endforeach
