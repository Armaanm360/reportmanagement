...
<div class="content">
    ...
    <div class="page-inner">
           <form action="{{ url('update-form-2.2.5') }}" id="submitForm" method="POST">
            @csrf

             <input type="hidden" name="author_id" value="{{ Auth::user()->id }}">
            <div class="rel-26">
                <h3>২.২.৫ তথ্য অধিকার বিষয়ে , <input type="hidden" name="years" id="yearTextInput" value="">  <span id="yearText"> @foreach ($years as $years) {{ $years -> years }}@endforeach</span>অর্থবছরের বার্ষিক কর্মপরিকল্পনা</h3>   
                
                     <label for="">Form ID</label>
                     {{-- @foreach ($all_form_id as $itemid)
                         
                    
                    <input type="text" value="{{ $itemid -> unique_form_id }}">
                      ...
                      <thead>
                       ...
                       <tbody>
                        ...
                        <tr class="row1">
                            ...
                            <td class="column2 style7 s"><textarea type="text" name="unit_{{$row->id}}" id="">{{$row->unit}}</textarea></td>
                            ...
                            <td class="column7 style6 s">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" id="" name="branch_responsible_implementation_{{$row->id}}">
                                        ...
                                        --}}
                                        <option value="{{ $division -> id }}" {{ ( $division -> id == $row->branch_responsible_implementation) ? 'selected' : '' }}> {{ $division -> name }} @endforeach </option>
                                    </select>
                                </div>
                            </td>
                        </tr>
