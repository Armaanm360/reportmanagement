...
@section('content')
    ...
    <div class="content">
        ...
        <div class="page-inner">
            <div class="page-header">
                <h4 class="page-title">{{ __('Regulatory Authority') }}</h4>
                <ul class="breadcrumbs">
                    ...
                    <li class="nav-item">
                        <a href="#">{{ __('Regulatory Authority') }}</a>
                    </li>
                    <li class="separator">
                        <i class="flaticon-right-arrow"></i>
                    </li>
                    <li class="nav-item">
                        <a href="#">{{ __('List of Regulatory Authority') }}</a>
                    </li>
                </ul>
            </div>
            <div class="row">
                <div style="width: 100%;text-align: right;float: right;margin-bottom: 7px"><button type="button"
                        ...
                        <i class="la flaticon-plus" style="font-size: 18px"></i> {{ __('Add Regulatory Authority') }}
                    </button></div>
                {{-- @if ($user->can('add-regularity-authority'))
                ...
                <div class="col-md-12">
                    ...
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">{{ __('List of Regulatory Authority') }}</h4>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table id="basic-datatable" class="display table table-striped table-hover">
                                    <thead>
                                        <tr>
                                            <th>{{ __('Sl') }}</th>
                                            <th>{{ __('Regulatory Authority') }}</th>
                                            <th>{{ __('Description') }}</th>
                                            <th>{{ __('Action') }}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($all_regularity as $row)
                                            <tr>
                                                <td>
                                                    {{ $loop->iteration }}
                                                </td>
                                                ...
                                                <td>
                                                    ...
                                                    {{-- @if ($user->can('edit-regularity-authority'))
                                            ...
                                            @endif
                                            @if ($user->can('delete-regularity-authority'))
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLongTitle">{{ __('Add Regularity Authority') }}</h5>
        ...
        <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="editModalTable"
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editModalTable">Edit Regularity Authority</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
    ...
    <script type="text/javascript">
        ...
        $("#add_form").submit(function(e) {
            ...
            $.ajax({
                ...
                window.location.href = "{{ url('regularityauthority') }}";
                // location.reload();
    ...
    <script type="text/javascript">
        $(".view_modal").click(function() {
            ...
            $.ajax({
                method: "GET",
                url: "regularityauthority/" + id + "/edit",
                data: id,
                // cache: false,
                // contentType: false,
                // processData: false,
            ...
            if (confirm('Are you sure?', true)) {
                $.ajax({
                    ...
                    success: function(data) {
                        alert('Hi');
                        notifiAlert('Alert!', 'Regularity Authority deleted successfully');
                        return false;
                    }
                });
                //                  alert('Hi');
